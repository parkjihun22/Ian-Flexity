...
const menuArray = [
  ...
  },
  {
    title: "세대안내",
    subMenu: [
      { subTitle: "49㎡", subUrl: "/FloorPlan/59A" },
  ...
  },
  {
    title: "인테리어",
    subMenu: [
      { subTitle: "84㎡", subUrl: "/Interior/59A" },
      ...
      <>
        ...
        <div className={styles.mobileHeader}>
          <div onClick={() => setIsMobileMenu((v) => !v)}>
            {!isMobileMenu ? (
              <AiOutlineMenu className={styles.icon} size={25} color="#014756" />
            ) : (
              <IoCloseSharp className={styles.icon} size={25} color="#014756" />
            )}
          </div>
          {isMobileMenu && (
            <SlideMenu
          ...
          <a href="tel:1533-8848">
            <IoCall className={styles.icon} size={25} color="#014756" />
          </a>
        </div>
        {isInterestPopupOpen && (
          <InterestPopup
